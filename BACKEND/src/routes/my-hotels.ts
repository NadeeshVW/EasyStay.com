...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const imageUrls = await uploadImages(imageFiles); //wait before giving the url for the images to get uploaded
...
router.get("/:id", verifyToken, async (req: Request, res: Response) => {
  ...
  try {
    const hotel = await Hotel.findOne({
      //It's used to retrieve a single document from a collection that meets specific criteria.
      _id: id,
      userId: req.userId,
    });
    res.json(hotel);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await hotel.save();
      res.status(201).json(hotel); //makes sure update complete.
